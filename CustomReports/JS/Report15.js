...
$(function () {
    ...
    $("#btnSaveAsImage").click(function () {
        var chart = $("#divGlobalReportHTML115").getKendoChart();
...
function createChart(strCharts) {
    $("#divGlobalReportHTML115").kendoChart({
        dataSource: {
            data: eval(strCharts)
        },
        title: {
...
function doReportCall() {
    ...
    $.ajax({
        type: "POST",
        url: "Report15.aspx/RefreshGlobalReport15",
        data: params,
        contentType: "application/json; charset=utf-8",
        beforeSend: function (XMLHttpRequest) {
            $('#divGlobalReportHTML115').html("<h5 style='color:red;'>正在生成报表...</h5>");
        },
        success: function (msg) {
            $('#divGlobalReportHTML115').html("");
            if (msg.d) {
                var strCharts = msg.d.toString();
                $("#divGlobalReportHTML115").ready(createChart(strCharts));
                $("#divGlobalReportHTML115").bind("kendo:skinChange", createChart(strCharts));
            }
        },
